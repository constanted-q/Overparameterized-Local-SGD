...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv_shortcut = nn.Conv2d(in_planes, self.expansion * planes,
    ...
    def shortcut(self, x):
        ...
        if self.stride != 1 or self.in_planes != self.expansion * self.planes:
            ...
            shape_temp = out.shape[1:]
            ln = nn.LayerNorm(shape_temp, elementwise_affine=False)
            out = ln(out)
            return out
        else:
            return x
    ...
    def forward(self, x):
        out = self.conv1(x)
        shape_temp = out.shape[1:]
        ln1 = nn.LayerNorm(shape_temp,elementwise_affine=False)
        out = F.relu(ln1(out))
        #out = F.relu(self.bn1(self.conv1(x)))
        out = self.conv2(out)
        shape_temp = out.shape[1:]
        ln2 = nn.LayerNorm(shape_temp, elementwise_affine=False)
        out = ln2(out)
...
class Bottleneck(nn.Module):
    ...
    '''def SelfNorm(self,x,planes,track_running_stats = False,affine = False):
        basic_norm = nn.BatchNorm2d(planes,track_running_stats,affine)
        for i in x.size(0):
            x[i] = basic_norm(x[i].clone)
        return x'''
